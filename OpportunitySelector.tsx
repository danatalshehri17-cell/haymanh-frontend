...
const OpportunitySelector: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch opportunities
      const opportunitiesResponse = await fetch('http://localhost:8000/api/opportunities?limit=20');
      ...
      // Fetch programs
      const programsResponse = await fetch('http://localhost:8000/api/programs?limit=20');
  ...
  const handleSelectItem = async (itemId: string, type: 'opportunity' | 'program') => {
    try {
      ...
      if (type === 'opportunity') {
        const response = await fetch('http://localhost:8000/api/dashboard/select-opportunity', {
          method: 'POST',
          headers: {
        ...
        const response = await fetch('http://localhost:8000/api/dashboard/enroll', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
