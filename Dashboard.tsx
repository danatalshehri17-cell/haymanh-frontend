...
const Dashboard: React.FC = () => {
  ...
  //   try {
  //     const token = localStorage.getItem('haymanh_token');
  //     const response = await fetch(`http://localhost:8000/api/dashboard/selected-opportunities/${opportunityId}`, {
  //       method: 'DELETE',
  //       headers: {
  //         'Authorization': `Bearer ${token}`,
  //         'Content-Type': 'application/json'
            ...
            {dashboardData?.userProgress?.selectedOpportunities?.length > 0 ? (
              <ProgramsGrid>
                {dashboardData.userProgress.selectedOpportunities.map((opportunity: any, index: number) => (
                  <ProgramCard
                    ...
                    whileHover={{ scale: 1.02 }}
                  >
                    <ProgramTitle>{opportunity.opportunityId?.title || opportunity.opportunityTitle || 'فرصة غير محددة'}</ProgramTitle>
                    <ProgramProgress>
                      <ProgressBar>
              ...
              <div style={{ 
                ...
                <p style={{ 
                  ...
                  fontSize: '1.1rem',
                  lineHeight: '1.6'
                }}>ابدأ باختيار الفرص المناسبة لك!</p>
              </div>
            )}
          </Section>
